...
int main(int argc, char **argv) {
  ...
  if (base_rank == root) {
    ...
    srand(time(NULL) + base_rank); // Seed random gen for tsunameters
    ...
    while (!test_mpi_req(&termination_req, &termination_flag,
      ...
      if (get_moving_avg(avg) > THRESHOLD) {
        ...
        printf("%d finished checking,  < %d num_neighours, time = %f - %f = "
               "%f < %d\n",
               base_rank, num_neighbours, endtime, starttime,
